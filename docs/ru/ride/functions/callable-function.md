...
В dApp-скрипте можно определить вызываемую функцию по умолчанию. Функция по умолчанию должна иметь имя `default` и не иметь аргументов:
...
Если в [транзакции вызова скрипта](/ru/blockchain/transaction-type/invoke-script-transaction) не указано поле `call`, вызывается функция по умолчанию.
