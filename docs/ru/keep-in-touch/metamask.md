...
:warning: Если из seed-фразы MetaMask сгенерировать пару ключей и адрес по правилам Waves (см. раздел [Cryptographic practical details](/en/blockchain/waves-protocol/cryptographic-practical-details)), получится **другой** аккаунт. Его адрес не будет совпадать с адресом пользователя MetaMask, преобразованным в формат Waves.
