...
Функция `Subscribe` возвращает все события по мере появления:
* до текущей высоты блокчейна — исторические данные, то есть сообщения об добавлении блока,
...
Функция возвращает массив `GetBlockUpdatesRangeResponse`, содержащий сообщения об изменениях `BlockchainUpdated`. Если указана конечная высота больше текущей, функция вернет ответ только в момент, когда высота блокчейна достигнет указанной конечной высоты. До текущей высоты возвращаются сообщения о добавлении блока, на текущей высоте могут появиться сообщения о любых событиях.
