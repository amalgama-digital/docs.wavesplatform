...
**Матчер** — сервис, который выполняет [ордера](#ордер) на покупку и продажу токенов и формирует транзакции обмена. Пример матчера — матчер Waves.Exchange, разработанный сторонней командой из сообщества, см. [документацию Waves.Exchange](https://docs.waves.exchange/ru/waves-matcher/).
...
**Ордер** (или биржевая заявка) — инструкция [матчеру](#матчер) на покупку или продажу [токена](#токен).
