...
Транзакция содержит два встречных [ордера](/ru/blockchain/order): ордер на покупку и ордер на продажу. Блокчейн гарантирует, что условия обмена не хуже, чем указаны в каждом из ордеров.
...
Как правило, транзакция обмена создается матчером — сервисом, который выполняет биржевые ордера на покупку и продажу токенов. Пример матчера — матчер Waves.Exchange, разработанный сторонней командой из сообщества, см. [документацию Waves.Exchange](https://docs.waves.exchange/ru/waves-matcher/).
...
:warning: В случае если ордер отправлен со смарт-аккаунта или комиссия матчера задана в смарт-ассете, минимальная комиссия за транзакцию **не увеличивается**.
