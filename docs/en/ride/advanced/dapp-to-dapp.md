...
The return value is of type `Any`, which means any valid type. You can extract a particular type from it using<!-- `as[T]` and `exactAs[T]` macros or --> the `match ... case` operator, see the [example](/en/ride/functions/built-in-functions/dapp-to-dapp#example).
