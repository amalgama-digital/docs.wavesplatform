...
You can get the list of features and their statuses using the [GET /activation/status](https://nodes.wavesnodes.com/api-docs/index.html#/activation/getFeatureStatuses) operation of [Node REST API](/en/waves-node/node-api/).
...
:warning: The following feature is available in node versions 1.4.x currently supported on Stagenet only:

| Feature ID | Name | Description |
|---|:---:|---|
| 17 | Ride V6, MetaMask support, Invoke Expression | <br>• Ride Standard library version 6.<br>• Added support for Ethereum transactions that perform token transfers or dApp script invocations and orders with ECDSA signature in Exchange transactions.<br>• Added Invoke Expression transaction type.<br>See complete list of changes in the [Release Notes](/ru/keep-in-touch/release-notes) article |
